...
def fingerprint(Adj):
    ...
    g_c = g.subgraph(largest_component).copy() # graph induced by largest connected  component

    measures = {'density': nx.density(G),
                'frac_largest_connected_component': frac_largest_component,
                'connected_components_ent': ent_components,
                'frac_center': fraction_center(g_c),
                'frac_periphery' : fraction_periphery(g_c),
                'frac_barycenter': fraction_barycenter(g_c),
                'srank_ratio' : normalized_srank(G),
                'erank_ratio': normalized_erank(G),
                'transitivity': nx.transitivity(G),
                'avg_clustering': nx.average_clustering(G),
                'avg_square_clustering': average_square_clustering(g),  
                'modularity': modularity_analysis(g),
                'markov_rate': markov_analysis(g),
                'basic_stats': dict_to_vec(basic_stats(Adj))[:-1],
                'eccentricity': eccentricity_analysis(g_c),
                'degree_analysis': normalized_degree_analysis(G),
                'flow_graph': flow_graph_analysis(g),
                'communicability': communicability_analysis(g_c),
                'similarity_analysis': similarity_analysis(g_c)
               }
...
def basic_stats(data, with_entropy = False):
    ...
    The output contains: 
        ...
        - a measure of uncertainty, the nornalized entropy, varying between 0 (sure result) and 1 (perferclty uniform ditribution)
    ...
    statistics = {'med': med, 'avg': avg, 'std': std, 'md': md, 'tau_3': tau_3, 'tau_4': tau_4, 'ent': ent}
    return statistics
